// Auto-Git UI
use auto_git::{lib_get_project_path, lib_git_update_local};
        ...
        loop {
            {
                ...
                *status = AppState::Searching;
                ...
                if files_staged.is_empty() {
                    shared_data.terminal_output.lock().unwrap().push("▶ Adding the modified files to the commit...".to_string());
                }
                ...
                )
                {
                    ...
                    Err(_) => {
                        *status = AppState::Error("Error during file staging execution".to_string());
                        shared_data.terminal_output.lock().unwrap().push(format!("{ERROR_TICK} Error in the file staging operation"));
                    }
                }
                // -------------------------------------------------------------------------------
            }
                ...
                *shared_data.status_output.lock().unwrap() = lib_git_update_local(&shared_data.project_path.lock().unwrap());
                ...
                if let Some(out) = shared_data.status_output.lock().unwrap().clone() {
                        ...
                        )
                        {
                            Ok(_) => {
                                *status = AppState::Success("Pull completed successfully ".to_string());
                                ...
                                shared_data.terminal_output.lock().unwrap().push(format!("{OK_TICK} Pull completed!"))
                            }
                            Err(_) => {
                                *status = AppState::Error("Error during pull execution".to_string());
                                
                                shared_data.terminal_output.lock().unwrap().push(format!("{ERROR_TICK} Errore nell'operzione di pull"));
                            }
                         }
                    ...
                    if shared_data.project_path.lock().unwrap().is_none() {
                        *status = AppState::Error("Set the project path from the Menu".to_string());
                        shared_data.terminal_output.lock().unwrap().push(format!("{ERROR_TICK} Error executing the status command: Set the project path"));
                    } else {
                        *status = AppState::Error("Status not completed".to_string());
...
impl AutoGitApp {
    ...
    fn handle_push(&mut self) {
        ...
        self.add_terminal_output("\u{2714} Starting operation Push".to_string());
                    ...
                    match err {
                        Error::Io(_) => {
                            *self.shared_data.state.lock().unwrap() = AppState::Error("Project path not set".to_string());
                            self.shared_data.terminal_output.lock().unwrap().push(format!("{ERROR_TICK} Project path not selected, configure one from the 'Set project' menu"));
                        },
                        _ => {
                            *self.shared_data.state.lock().unwrap() = AppState::Error("Push operation not completed due to an error".to_string());
                            self.shared_data.terminal_output.lock().unwrap().push(format!("{ERROR_TICK} Push operation failed due to an error: {}", err));
                        }
                    }
                }
            }
    ...
    fn handle_ignore(&mut self) {
        ...
        self.add_terminal_output("\u{2714} Local project changes ignored".to_string());
        
        if let Ok(mut state) = self.shared_data.state.try_lock() {
            in_error_state = matches!(*state, AppState::Error(_));
            *state = AppState::Success("Local project changes ignored successfully!".to_string());
        }
    ...
    fn handle_status(&mut self) {
        ...
        match out.lock().unwrap().clone() {
            Some(output) => {
                ...
                if let Ok(mut state) = self.shared_data.state.try_lock() {
                    in_error_state = matches!(*state, AppState::Error(_));
                    *state = AppState::Success("Status completed successfully".to_string());
                }
    ...
    fn draw_menu(&mut self, ui: &mut egui::Ui) {
        egui::MenuBar::new().ui(ui, |ui| {
            ui.menu_button(RichText::new("☰ Menu").size(17.0), |ui| {
                ...
                if ui.button("Reset")
                    .highlight()
                    .on_hover_text("Clear the errors by resetting the app state")
                    .clicked()
                {
        ...
        ui.horizontal(|ui| {
            ...
            match state {
                ...
                AppState::Searching => {
                    ...
                    ui.add_space(5.0);
                    ui.label(
                        egui::RichText::new("Checking changes...")
                            .size(14.0)
                            .color(egui::Color32::from_rgb(33, 150, 243))
                    );
                }
    ...
    fn draw_terminal_panel(&mut self, _ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        egui::TopBottomPanel::bottom("Teminal Output Buttons")
            .exact_height(40.0)
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                // ui.add_space(10.0);
                        ...
                        .show(ui, |ui| {
                            ...
                            if output.is_empty() {
                                ui.horizontal(|ui| {
                                    ui.add_space(10.0);
                                    ui.label(
                                        egui::RichText::new("No output available")
                                            .size(12.0)
                                            .color(egui::Color32::GRAY)
                                            .italics()
                                    );
    ...
    fn draw_commit_input(&mut self, ui: &mut egui::Ui) {
        ...
        if self.commit_input_expanded {
            ui.label(RichText::new("Messaggio del commit").size(15.).strong());
            ...
            ui.vertical_centered(|ui| {
                ...
                if ui.add(push_button).clicked() {
                    self.add_terminal_output(format!("▶ Aggiungo elementi al commit con messaggio: '{}'", self.commit_message));
